...
export function Stats() {
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-2xl font-black">{t("stats.title")}</h2>

      <section className="card-surface p-4">
        <div className="flex items-center justify-between mb-1">
          <h3 className="font-bold accent-blue">{t("stats.bestThirds")}</h3>
            ...
            return (
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="text-xl">{third.standing.team.flag}</span>
                  <span className="font-semibold">{third.standing.team.name}</span>
                  <span className="text-xs text-muted-foreground">({third.group})</span>
                </span>
                <span className="text-xs flex items-center gap-2">
                  <span><b className="accent-red">{third.standing.pts}</b> {t("stats.pts")} · {t("groups.dg")} {third.standing.dg}</span>
                  {eliminated && (
      ...
      <LeaderTable kind="goals" />
      <LeaderTable kind="assists" />
    </div>
  );
}

// Ranking REAL del Mundial (goleadores o asistencias) desde la API de ESPN.
function LeaderTable({ kind }: { kind: LeaderKind }) {
  const { t } = useI18n();
  const { rows, loading, error } = useLeaders(kind);
  const title = kind === "goals" ? t("stats.scorers") : t("stats.assists");
  const unit: [string, string] =
    kind === "goals"
      ? [t("stats.goalUnit1"), t("stats.goalUnit2")]
      : [t("stats.assistUnit"), t("stats.assistUnit")];
  return (
    <section className="card-surface p-4">
    ...
    <li className="flex items-center gap-2.5 py-1">
      ...
      <span className="flex flex-col min-w-0 flex-1">
        ...
        <span className="flex items-center gap-1 text-[11px] text-muted-foreground truncate">
          ...
          {p.matches != null && <span className="opacity-70">· {p.matches} {t("stats.mp")}</span>}
        </span>
      </span>
