  ...
  return (
    <div className="space-y-4 animate-fade-up">
      ...
      {next && home && away && (
        <div className="card-surface p-4 relative">
          <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
            {isLive ? (
              ...
              <span className="flex items-center gap-1.5 font-bold">
                <CalendarDays className="h-3.5 w-3.5 accent-gold" /> {t("home.nextMatch")}
              </span>
            )}
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground/90">{t("common.group")} {next.group} · {t("home.matchday")} {jornada}</span>
              ...
              {isLive && nextFixture?.homeScore !== null && nextFixture?.awayScore !== null ? (
                <>
                  ...
                  <div className="text-[9px] uppercase tracking-wider text-primary font-bold mt-1">
                    {nextFixture?.clock ? t("home.minute", { clock: nextFixture.clock }) : t("home.inPlay")}
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className="text-2xl font-black">{t("common.vs")}</div>
                  <div className="text-[9px] uppercase tracking-wider text-muted-foreground">{t("home.localTime")}</div>
                </>
              )}
            </div>
            <div className="flex flex-col items-center gap-2 text-center">
          ...
          >
            ...
            <span className="relative z-10">{featuredEventId ? t("home.viewDetails") : t("home.viewMatch")}</span>
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-2">
        <StatTile icon={CalendarDays} value={played} label={t("home.stat.played")} sub={t("home.stat.playedSub")} tone="red" />
        <StatTile icon={Goal} value={goals} label={t("home.stat.goals")} sub={t("home.stat.goalsSub")} tone="blue" />
        <StatTile icon={Users} value={qualified} label={t("home.stat.qualified")} sub={t("home.stat.qualifiedSub")} tone="green" />
        <StatTile icon={Trophy} value={champTeam ? "1" : "0"} label={t("home.stat.champion")} sub={champTeam?.name ?? t("common.tbd")} tone="gold" />
      </div>
      ...
      <div className="card-surface p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-black uppercase tracking-wider flex items-center gap-2">
            <Users className="h-4 w-4 accent-gold" /> {t("home.groupStage")}
          </h3>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-[11px] font-black uppercase tracking-wider ${tone.text}`}>{t("common.group")} {g}</span>
                ...
                <div className={`mt-2 pt-2 border-t border-white/10 text-[10px] font-bold flex items-center justify-center gap-1 ${tone.text}`}>
                  <ChevronRight className="h-3 w-3" /> {t("home.viewTable")}
                </div>
              </div>
            );
          })}
      ...
      <div className="card-surface p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-black uppercase tracking-wider flex items-center gap-2">
            <Trophy className="h-4 w-4 accent-gold" /> {t("home.finalBracket")}
          </h3>
          <button onClick={() => go("bracket")} className="text-xs text-muted-foreground flex items-center gap-1 font-bold press">
            {t("home.viewBracket")} <ChevronRight className="h-3 w-3" />
          </button>
        </div>
        <BracketPreview />
      </div>
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-center">
      ...
      <div className="flex flex-col items-center gap-1 px-1">
        ...
        <div className="text-[10px] font-black uppercase tracking-widest accent-gold">{t("common.final")}</div>
        <div className="text-[9px] text-muted-foreground text-center">
          {finalM?.homeScore !== null && finalM?.awayScore !== null ? t("common.finished") : t("common.tbd")}
        </div>
      </div>
      <div className="space-y-2">
        {tR.map((t, i) => (
...
function MiniSlot({ team, side }: { team: any; side: "left" | "right" }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[11px] font-bold truncate flex-1">{team?.name ?? t("common.tbd")}</div>
    </div>
  );
}
